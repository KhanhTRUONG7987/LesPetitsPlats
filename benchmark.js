...
const algo1 = (recipes, searchQuery) => {
...
// Arrays to store the results of all test cases
const resultsAlgorithm1 = [];
const resultsAlgorithm2 = [];

// Test Case 1: Benchmark the first algorithm
suite.add("Algorithm 1 (Single Word Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "coco")); // Single Word Query
});

suite.add("Algorithm 1 (Multiple Words Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "limonade de coco")); // Multiple Words Query
});

suite.add("Algorithm 1 (Partial Word Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "cuill")); // Partial Word Query
});

suite.add("Algorithm 1 (Empty Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "")); // Empty Query
});

suite.add("Algorithm 1 (Complex Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "jus de citron vert avec le citron")); // Complex Query
});

suite.add("Algorithm 1 (Non-Existent Query)", () => {
  resultsAlgorithm1.push(algo1(recipes, "xyz123luv")); // Non-Existent Query
});

// Test Case 2: Benchmark the second algorithm
suite.add("Algorithm 2 (Single Word Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "coco")); // Single Word Query
});

suite.add("Algorithm 2 (Multiple Words Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "limonade de coco")); // Multiple Words Query
});

suite.add("Algorithm 2 (Partial Word Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "cuill")); // Partial Word Query
});

suite.add("Algorithm 2 (Empty Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "")); // Empty Query
});

suite.add("Algorithm 2 (Complex Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "jus de citron vert avec le citron")); // Complex Query
});

suite.add("Algorithm 2 (Non-Existent Query)", () => {
  resultsAlgorithm2.push(algo2(recipes, "xyz123luv")); // Non-Existent Query
});
...
suite
  ...
  .on("complete", () => {
    // Calculate the total number of operations for each algorithm
    const totalOpsAlgorithm1 = resultsAlgorithm1.reduce(
      (total, result) => total + result.length,
      0
    );
    const totalOpsAlgorithm2 = resultsAlgorithm2.reduce(
      (total, result) => total + result.length,
      0
    );

    // Display the total number of operations for each algorithm
    console.log("\nTotal Operations for Algorithm 1:", totalOpsAlgorithm1);
    console.log("Total Operations for Algorithm 2:", totalOpsAlgorithm2); 

    // Compare the overall performance of the two algorithms
    if (totalOpsAlgorithm1 < totalOpsAlgorithm2) {
      console.log("\nAlgorithm 1 is faster.");
    } else if (totalOpsAlgorithm1 > totalOpsAlgorithm2) {
